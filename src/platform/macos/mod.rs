...
use objc2_app_kit::{
    ...
    NSWindowStyleMask, NSWindowTabbingMode, NSWindowTitleVisibility, NSWorkspace,
};
    ...
    fn remove_system_show_all_tabs(menu: &NSMenu) {
        ...
        while idx > 0 {
            ...
            if let Some(item) = menu.itemAtIndex(idx) {
                ...
                if !should_show_native_tab_bar() && title_ref == ns_string!("Merge All Windows") {
                    menu.removeItemAtIndex(idx);
                    continue;
                }

                if !should_show_native_tab_bar() && title_ref == ns_string!("Show Tab Bar") {
                    menu.removeItemAtIndex(idx);
                    continue;
                }
