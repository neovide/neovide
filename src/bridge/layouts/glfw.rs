...
pub fn handle_qwerty_layout(
    ...
    match (keycode, shift, ctrl, alt) {
        ...
        (Keycode::Apostrophe, _, _, _) => unsupported_key(Keycode::Apostrophe),
        (Keycode::Equal, _, _, _) => unsupported_key(Keycode::Equal),
        (Keycode::GraveAccent, _, _, _) => unsupported_key(Keycode::GraveAccent),
        (Keycode::KpDecimal, _, _, _) => unsupported_key(Keycode::KpDecimal),
        (Keycode::World1, _, _, _) => unsupported_key(Keycode::World1),
        (Keycode::World2, _, _, _) => unsupported_key(Keycode::World2),
        (Keycode::Enter, _, _, _) => Some(("Enter", true, shift, ctrl, alt)),
        (Keycode::F25, _, _, _) => unsupported_key(Keycode::F25),
        (Keycode::NumLock, _, _, _) => unsupported_key(Keycode::NumLock),
        (Keycode::KpSubtract, _, _, _) => unsupported_key(Keycode::KpSubtract),
        (Keycode::KpAdd, _, _, _) => unsupported_key(Keycode::KpAdd),
        (Keycode::KpEqual, _, _, _) => unsupported_key(Keycode::KpEqual),
        (Keycode::LeftShift, _, _, _) => unsupported_key(Keycode::LeftShift),
        (Keycode::LeftControl, _, _, _) => unsupported_key(Keycode::LeftControl),
        (Keycode::LeftAlt, _, _, _) => unsupported_key(Keycode::LeftAlt),
        (Keycode::LeftSuper, _, _, _) => unsupported_key(Keycode::LeftSuper),
        (Keycode::RightShift, _, _, _) => unsupported_key(Keycode::RightShift),
        (Keycode::RightControl, _, _, _) => unsupported_key(Keycode::RightControl),
        (Keycode::RightAlt, _, _, _) => unsupported_key(Keycode::RightAlt),
        (Keycode::RightSuper, _, _, _) => unsupported_key(Keycode::RightSuper),
        (Keycode::Unknown, _, _, _) => unsupported_key(Keycode::Unknown),
    }
}
